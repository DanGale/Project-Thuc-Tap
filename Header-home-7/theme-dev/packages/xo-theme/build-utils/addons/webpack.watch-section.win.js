...
const { localesDefault } = require('../helpers/localesDefault.win');
const { buildSettingsData } = require('../helpers/settingsData');
const { watchSectionLiquid, watchSectionSCSS } = require('../helpers/watch.win');
const { buildAllSingleSCSS } = require('../helpers/watch');
const { watchSingleSCSS } = require('../helpers/watch');
      ...
      compiler.hooks.watchRun.tap('WatchSectionPlugin', (compilation) => {
        if (compilation.modifiedFiles) {
          ...
          for (let i = 0; i < changedFiles.length; i++) {
            ...
            if (
              changedFiles[i].includes('src\\scss\\single\\') ||
